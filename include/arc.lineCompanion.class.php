...
class LineCompanion
{
	...
	function __construct($s_type, $s_im, $s_over, $s_sex)
	{
	...
	{
		if ($this->TempSource == '') {
			$tempfile = $GLOBALS['cfg_basedir'] . $GLOBALS['cfg_templets_dir'] . "/" . $GLOBALS['cfg_df_style'] . "/line_companion.htm";
	...
	private function _getComOverSql()
	{
		switch ($this->ComOver) {
			case 0:
				return " and l.com_over=0";
			break;
			case 1:
				return " and l.com_over=1";
			break;
			default :
				return '';
			break;
	...
	private  function _getComImSql()
	{
		switch ($this->ComIm) {
			case 1:
				return ' and l.com_im=1';
				break;
			case 0:
				return ' and l.com_im=0';
				break;
			default:
				return '';
			break;
	...
	private function _getComSexSql()
	{
		switch ($this->ComSex) {
			case 1:
				return ' and l.com_sex=1';
				break;
			case 0:
				return ' and l.com_sex=0';
				break;
			default:
				return '';
				break;
	...
	{
		$cnt  = "select count(aid) as cnt from `#@__companion` l, `#@__archives` arc where l.aid = arc.id ";
		...
		if ($this->TotalResult) {
			$sql = "select l.aid, arc.title from `#@__companion` l, `#@__archives` arc where l.aid = arc.id ";
	...
	function GetPageListDM($list_len)
	{
		...
		if ($this->ComOver) {
			$geturl = "s_over=".urlencode($this->ComOver);
			$hidenform = "<input type='hidden' name='s_over' value='".rawurldecode($this->ComOver)."'>\r\n";
		}
		if ($this->ComSex) {
			$geturl .= "&s_sex=".$this->ComSex;
			$hidenform .= "<input type='hidden' name='s_sex' value='".$this->ComSex."'>\r\n";
		}
		if ($this->ComIm) {
			$geturl .= "&s_im=".$this->ComIm;
			$hidenform .= "<input type='hidden' name='s_im' value='".$this->ComIm."'>\r\n";
		}
		if ($this->TypeID) {
