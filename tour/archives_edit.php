...
else if($dopost=='save')
{
    ...
    ";
	if ($channelid == 17) {
		$msg = "<div style=\"line-height:36px;height:36px;text-align: center;\">进入下一步编辑旅行资料</div>";
		$win = new OxWindow();
		$win->AddTitle('成功发布文档：');
		$win->AddMsgItem($msg);
		$winform = $win->GetWindow('hand', '&nbsp;', false);
		$win->Display();
		exit('<meta http-equiv="refresh" content="2; url=' . urldecode("line_full.php?artid={$id}&do=ext_edit") . '"/>');
	}
    $wintitle = "成功更改文档！";
